...
import pickle
from classifier.utils import image as image_utils
...
class DatasetFromFile(Dataset):
	def __init__(self, path, data_file, img_size = None, data_format = "h5", transform = None):
		...
		elif data_format == "h5_class":
			self.data, self.label, = self.load_h5_class_data(path, data_file)
	...
	def load_h5_class_data(self, path, data_file, mu = [0.5], std=[0.5]):
...
def imagenetIndex_to_labels(index, data_path = "/data/UG2_data/", file1 = "map_clsloc.txt"):
	# Read the label_number
	# Map to label_name
	f = open(os.path.join(data_path, file1), 'r')
	x = f.readlines()
	f.close()   
	line = x[index]
	splitLine = line.split()
	imageNetlabel = splitLine[2]
